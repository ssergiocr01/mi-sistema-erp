...
import { Package } from "lucide-react"; // <--- Solución error 1
...
export default function RegisterPage() {
  ...
  // Solución error 2: Definir exactamente la función que llama el form
  const handleSubmit = async (formData: FormData) => {
    ...
    if (res?.error) {
      ...
      await Swal.fire("¡Éxito!", "Usuario administrador creado", "success");
      router.push("/login");
    }
  };
  ...
  return (
    <div className="w-full max-w-[400px] space-y-6 flex flex-col items-center">
      <div className="flex flex-col items-center space-y-2 text-center">
        ...
        <p className="text-slate-500">NexusERP - Gestión Profesional</p>
      </div>
        ...
        <CardContent className="pt-6">
          <form action={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label className="text-slate-700 font-semibold ml-1">Nombre</Label>
              <Input name="name" placeholder="Tu nombre" className="h-11 border-slate-200" required />
            </div>
            <div className="space-y-2">
              <Label className="text-slate-700 font-semibold ml-1">Email</Label>
              <Input name="email" type="email" placeholder="nombre@empresa.com" className="h-11 border-slate-200" required />
            </div>
            <div className="space-y-2">
